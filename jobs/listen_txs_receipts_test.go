...
type ListenTxsReceiptsSuite struct {
	suite.Suite
}

func (suite *ListenTxsReceiptsSuite) SetupTest() {
...
func (suite *ListenTxsReceiptsSuite) TestListenTxsReceipts() {
	...
	_, err = db.Conn.CommitOneRow("delete from `signs_detail` where id = ?", 12000002)
	assert.NoError(suite.T(), err)
}

func TestTxsReceiptsTxs(t *testing.T) {
	suite.Run(t, new(ListenTxsReceiptsSuite))
}
